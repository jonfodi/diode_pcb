...
struct EvalContextState {
    ...
    /// Map of `module.star` → set of files referenced via `load()`. Used by the LSP to
...
impl EvalContext {
    ...
    /// Build a synthetic module that exposes one `ModuleLoader` per `.star` file
    /// found directly inside `dir`.  This is used to implement the shorthand
    /// `load("path/to/folder", "Foo", "Bar")`, which behaves as if the
    /// caller had written `Foo = Module("path/to/folder/Foo.star")` for
    ...
    fn load_directory_as_module(
        ...
        // Gather all immediate *.star entries.
            ...
            .filter(|p| {
                file_provider.exists(p)
                    ...
                    && p.extension()
                        ...
                        .map(|ext| ext.eq_ignore_ascii_case("star"))
                        .unwrap_or(false)
            })
            .collect();
        ...
        for star_path in entries {
            ...
            // this becomes "@something/my/directory/file.star"
        ...
        let mut sym_entries: Vec<std::path::PathBuf> = dir_entries
            ...
            .filter(|p| {
                file_provider.exists(p)
                    ...
                    && p.extension()
                        .and_then(OsStr::to_str)
                        .map(|ext| ext.eq_ignore_ascii_case("kicad_sym"))
                        .unwrap_or(false)
            })
            .collect();
...
impl FileLoader for EvalContext {
    fn load(&self, path: &str) -> starlark::Result<starlark::environment::FrozenModule> {
        ...
        // namespace that exports one `ModuleLoader` per `.star` file found
