...
use pcb_star_core::{FileProvider, FileProviderError};
...
///     "file1.star" => "content1",
///     "file2.star" => "content2",
///     "main.star" => "main content"
/// });
/// ```
///
/// This will:
/// 1. Create an in-memory file system with the specified files
/// 2. Evaluate "main.star" (the last file in the list)
...
macro_rules! snapshot_eval {
    ($name:ident, { $($file:expr => $content:expr),+ $(,)? }) => {
        ...
        fn $name() {
            use std::sync::Arc;
            use pcb_star_core::{EvalContext, InputMap, WorkspaceLoadResolver};
