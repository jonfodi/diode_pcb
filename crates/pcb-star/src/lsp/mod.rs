...
use pcb_star_core::convert::ToSchematic;
...
fn create_standard_load_resolver(workspace_root: &Path) -> Arc<CompoundLoadResolver> {
...
impl Default for LspEvalContext {
    fn default() -> Self {
        ...
        for (name, item) in globals.documentation().members {
            if let Ok(url) = Url::parse(&format!("starlark:/{name}.star")) {
...
impl LspEvalContext {
    ...
    fn diagnostic_to_lsp(&self, diag: &pcb_star_core::Diagnostic) -> lsp_types::Diagnostic {
        use lsp_types::{
        ...
        match uri {
            LspUrl::File(path) => {
                ...
                let workspace_root = path
                    .parent()
                    .and_then(crate::load::find_workspace_root)
                    .unwrap_or_else(|| path.parent().unwrap_or(path).to_path_buf());

                // Create a load resolver for this file
                let load_resolver = create_standard_load_resolver(&workspace_root);
        ...
        match current_file {
            LspUrl::File(path) => {
                if let Some(target_path) = self.inner.get_url_for_global_symbol(path, symbol) {
                    ...
                    if let Ok(parsed_url) = Url::parse(&format!("starlark:/{symbol}.star")) {
                    ...
                    let state_json: Option<JsonValue> = match &params.uri {
                        LspUrl::File(path_buf) => {
                            ...
                            let ctx = EvalContext::new()
                                ...
                                .set_load_resolver(create_standard_load_resolver(&workspace_root));
