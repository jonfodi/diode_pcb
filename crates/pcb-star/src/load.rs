...
///   assumed.
///   Example: `"@stdlib:1.2.3/math.star"` or `"@stdlib/math.star"`.
///
/// • **GitHub repository** –
...
///   Example: `"@github/foo/bar:abc123/scripts/build.star".
///
...
mod tests {
    ...
    fn parses_package_without_tag() {
        let spec = parse_load_spec("@stdlib/math.star");
        assert_eq!(
            spec,
            Some(LoadSpec::Package {
                package: "stdlib".to_string(),
                tag: DEFAULT_PKG_TAG.to_string(),
                path: PathBuf::from("math.star"),
            })
        );
    }
    ...
    fn parses_github_with_rev_and_path() {
        let spec = parse_load_spec("@github/foo/bar:abc123/scripts/build.star");
        assert_eq!(
            spec,
            Some(LoadSpec::Github {
                user: "foo".to_string(),
                ...
                path: PathBuf::from("scripts/build.star"),
            })
        );
    }
    ...
    fn parses_github_without_rev() {
        let spec = parse_load_spec("@github/foo/bar/scripts/build.star");
        assert_eq!(
            spec,
            Some(LoadSpec::Github {
                user: "foo".to_string(),
